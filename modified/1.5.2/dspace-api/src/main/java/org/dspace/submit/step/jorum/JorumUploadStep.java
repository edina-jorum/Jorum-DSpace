...
public class JorumUploadStep extends AbstractProcessingStep {
	...
	/***Added by CG ****/
	/** Indicates a URL was not supplied **/
	public static final int STATUS_MISSING_URL = 6;
	public static final int ALL_BUNDLES_REMOVED = 7;
	public static final int SINGLE_BUNDLE_REMOVED = 8;
	public static final int VIRUS_CHECK_FAILED = 9;

	private static final String URL_IDENTIFIER = "identifier";

	// the metadata language qualifier
	private static final String LANGUAGE_QUALIFIER = BundleUtils.getDefaultLanguageQualifier();
	...
	public int doProcessing(Context context, HttpServletRequest request, HttpServletResponse response,
		...
		//Deal with URL submissions
		if (BundleUtils.checkUrl(subInfo)) {
			//Get url from request
			String url = request.getParameter("url");
			
			// GWaller 20/8/09 Url can be http, https, ftp etc - use utility class for this
			String trimmedURL = url.trim();
			int urlSchemeLen = URLChecker.isURL(trimmedURL);
			
			if (trimmedURL.length() == 0 || (trimmedURL.length() == urlSchemeLen) ) {
				// If url not entered in form, return error to prevent user from proceeding.  
				// Error picked up in JorumUploadStep xmlui class
				return STATUS_MISSING_URL;
			}

			Bundle bundle = getURLBundle(item);
			// set the format
			BitstreamFormat bs_format = BitstreamFormat.findByShortDescription(context, "Text");
			// set the URL as the primary bitstream
			// GWaller 20/8/09 Use trimmed URL not the user submitted one
			BundleUtils.setBitstreamFromBytes(bundle, trimmedURL, bs_format, trimmedURL.getBytes(), true);

			//Set the url in dc.identifier field associated with the object
			// GWaller 20/8/09 Use trimmed URL not the user submitted one
			BundleUtils.clearAndSetMetadataElement(trimmedURL, item, Constants.DC_SCHEMA, URL_IDENTIFIER, null, LANGUAGE_QUALIFIER);
			//All saved and ok - allow user to proceed to the next step
		...
		if (buttonPressed.equalsIgnoreCase("submit_remove_selected")) {
			...
			if (request.getParameter("remove") != null) {
				...
				for (int i = 0; i < removeIDs.length; i++) {
					...
					status = processRemoveFile(context, item, id, subInfo);
					// Added by CG
					// We should return status complete if all bundles removed and metatdata
					// deleted.  If not, red error message would have been displayed
					if(status == ALL_BUNDLES_REMOVED){
						subInfo.setBitstream(null);
						return STATUS_COMPLETE;
					}			
				}
				...
				// if one or more (but not all removed) we can just return status_complete, otherwise red error message
				// would be displayed
				if(status == SINGLE_BUNDLE_REMOVED){
					return STATUS_COMPLETE;
				}
			}
			...
			int status = processRemoveFile(context, item, id, subInfo);
	 ...
	 *         no errors occurred!)
	 */
	protected int processRemoveFile(Context context, Item item, int bitstreamID, SubmissionInfo subInfo)
		...
		try {
			...
			status = checkRelated(context, item, bitstreamID, subInfo);

			// Now check the wrapper item
			status = checkWrapper(context, item, bitstreamID, subInfo, status);

			// A final check to ensure that the workspace position is reset
			int bundlesLength = item.getBundles().length;
			if (item.getBundles("ORIGINAL").length > 0) {
				// if there are no bitstreams in the Original bundle and no other bundles, 
				// reset stage reached and return ALL_BUNDLES_REMOVED
				if ((item.getBundles("ORIGINAL")[0].getBitstreams().length == 0 && bundlesLength == 1)) {
					resetStageReached(subInfo);
					status = ALL_BUNDLES_REMOVED;
				}
			} else if (bundlesLength == 0) {
				// If there are no bundles at all, reset the stage reached
				resetStageReached(subInfo);
				status = ALL_BUNDLES_REMOVED;
			}
	...
	/**
	 * Cycle through the wrapper bitstreams to check for a match and remove
	 * as appropriate 
	 * @param context
	 * @param item
	 * @param bitstreamID
	 ...
	 * @throws IOException
	 */
	private int checkWrapper(Context context, Item item, int bitstreamID, SubmissionInfo subInfo, int status)
			throws SQLException, AuthorizeException, IOException {
		
	
		Bundle[] wrapperBundles = item.getBundles();
		for (Bundle bundle : wrapperBundles) {
			Bitstream[] wrapperBitstreams = bundle.getBitstreams();
			for (Bitstream bStream : wrapperBitstreams) {
				int wrapperBSID = bStream.getID();
				// check if there's a bitstream in the wrapper object that should be removed
				if (bitstreamID == wrapperBSID) {
					status = removeWrapperBitstream(context, item, bundle, bStream, subInfo);
					if (status != ALL_BUNDLES_REMOVED) {
						status = SINGLE_BUNDLE_REMOVED;
					}
				}
			}
		}
	...
	/**
	 * We start with the wrapper item and the id of the bitstream to be deleted from the related item
	 * Cycle through relatedCPs of wrapper item
	 * For each, get the name (which is the handle of the related item)
	 * Locate related item using this handle and get the bitstreams in the archivedCP of related item
	 * Check if bitstream id matches the one to be deleted
	 * If so, delete the related item and then the relatedCP bitstream in the wrapper item  
	 * 
	 * @param context
	 * @param item
	 * @param bitstreamID
	 ...
	 * @throws IOException
	 */
	private int checkRelated(Context context, Item item, int bitstreamID, SubmissionInfo subInfo)
			throws SQLException, AuthorizeException, IOException {
		
		int status = 0;
		Bundle[] related = item.getBundles(Constants.RELATED_CONTENT_PACKAGE_BUNDLE);
		if (related.length > 0) {
			//Get the actual archived cp bundles from the related CPs
			for (Bundle bundle : related) {
				Bitstream[] bitstreams = bundle.getBitstreams();
				// Cycle through the related items
				// for each, get the ArchivedCP bundle
				for (Bitstream relatedBitstream : bitstreams) {
					// Get the name of the related item
					Item relatedItem = (Item) HandleManager.resolveToObject(context, relatedBitstream.getName());
					Bundle archived = relatedItem.getBundles(Constants.ARCHIVED_CONTENT_PACKAGE_BUNDLE)[0];
					Bitstream bs = archived.getBitstreams()[0];
					int id = bs.getID();
					if (id == bitstreamID) {
						// remove this item from appropriate collections
						// (if we don't do this we get a referential integrity db error)
						Collection[] collections = relatedItem.getCollections();
						for (Collection c : collections) {
							c.removeItem(relatedItem);
						}

						relatedItem.delete();

						// and remove the relatedCP bitstream that links to it
						status = removeRelatedBitstream(context, item, bundle, relatedBitstream);
	...
	/**
	 * 
	 * Remove bitstream from wrapper object
	 * Delete all wrapper bundles/metadata if appropriate
	 * 
	 * @param context
	 * @param item
	 * @param bundle
	 ...
	 * @throws IOException
	 */
	private int removeWrapperBitstream(Context context, Item item, Bundle bundle, Bitstream bitstream,
			SubmissionInfo subInfo) throws AuthorizeException, SQLException, IOException {

		int length = removeSingleBitstream(item, bundle, bitstream);

		//Check if relatedCP exists
		Bundle[] related = item.getBundles(Constants.RELATED_CONTENT_PACKAGE_BUNDLE);
		String name = bundle.getName();
		if (name.equals(Constants.ARCHIVED_CONTENT_PACKAGE_BUNDLE)
				|| (name.equals("ORIGINAL") && length == 0 && related.length == 0)) {
			// Delete all bundles, bitstreams and metadata and reset workflow stage reached
	...
	private int removeRelatedBitstream(Context context, Item item, Bundle bundle, Bitstream bitstream)
			throws AuthorizeException, SQLException, IOException {

		removeSingleBitstream(item, bundle, bitstream);
		context.commit();
		//item.update();

		return SINGLE_BUNDLE_REMOVED;
	}
	
	/**
	 * 
	 * Removes bitstream from specified bundle
	 * and removes bundle if empty
	 * 
	 * @param item
	 * @param bundle
	 * @param bitstream
	 ...
	 * @throws IOException
	 */
	private int removeSingleBitstream(Item item, Bundle bundle, Bitstream bitstream) throws AuthorizeException,
			SQLException, IOException {
		bundle.removeBitstream(bitstream);
		int length = bundle.getBitstreams().length;
		if (length < 1) {
	...
	/**
	 * Reset the workflow stage - disables workflow submission stage buttons appropriately
	 * @param subInfo
	 */
	private void resetStageReached(SubmissionInfo subInfo) {
		WorkspaceItem workspaceItem = (WorkspaceItem) subInfo.getSubmissionItem();
		if (workspaceItem.getStageReached() > 2) {
			workspaceItem.setStageReached(2);
		}
	}
	...
	protected int processUploadFile(Context context, HttpServletRequest request, HttpServletResponse response,
		...
		while (attNames.hasMoreElements()) {
			...
			if (attr.endsWith("-path")) {
				...
				if (filePath == null || fileInputStream == null) {
				    ...
				    // GH - virus check file
				    if (Boolean.valueOf(ConfigurationManager.getProperty("enable.viruscheck"))){
				        
				        File file = new File(request.getParameter("file"));
				        if(!new VirusChecker(
				                ConfigurationManager.getProperty("clamscan.path").trim(),
				                file).isVirusFree()){
				            return VIRUS_CHECK_FAILED;
				        }
				    }
				    // GH - end
				}
		...
		if (!formatKnown) {
			/* GWaller 6/11/09 IssueID #131 This used to return STATUS_UNKNOWN_FORMAT for an unknown bitstream
			                 format. However when this happens, the Javascript controller, redisplays the upload 
			                 step but lists the file uploaded at the bottom. Inorder to get to describe step (for 
			                 metadata), the user must hit "Next" again. This coudl be confusing for the user esp
			                 since we still take the content even if it is of an unknown type.  Better to simply log
			                 the unknown format (so we can track what the users are inputing) and return
			                 STATUS_COMPLETE so they can go to the next step as the user intended.
			*/
			log.warn("Unknown file format deposited, filename was " + noPath);
			return STATUS_COMPLETE;
		} else
			return STATUS_COMPLETE;
	...
	/**
	 * Helper method to create a new URL_BUNDLE and delete any old ones
	 * 
	 * @param item 	The submission Item
	 * @return Bundle suitable for storing a url
	 * @throws SQLException
	 * @throws AuthorizeException
	 * @throws IOException
	 */
	public static Bundle getURLBundle(Item item) throws SQLException, AuthorizeException, IOException {
		// GWaller 25/09/09 URL_BUNDLE name now defined in Constants class
		Bundle[] bundles = item.getBundles(Constants.URL_BUNDLE);

		if ((bundles.length > 0) && (bundles[0] != null)) {
			item.removeBundle(bundles[0]);
		}
		return item.createBundle(Constants.URL_BUNDLE);
	}
